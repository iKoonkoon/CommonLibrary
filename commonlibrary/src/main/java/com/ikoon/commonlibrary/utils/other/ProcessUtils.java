        ...
        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.LOLLIPOP) {
            ...
            try {// 有"有权查看使用权限的应用"选项
                ApplicationInfo info =
                        packageManager.getApplicationInfo(AppUtils.getApp().getPackageName(), 0);
                AppOpsManager aom =
                        (AppOpsManager) AppUtils.getApp().getSystemService(Context.APP_OPS_SERVICE);
                if (aom != null) {
                ...
                UsageStatsManager usageStatsManager = (UsageStatsManager) AppUtils.getApp()
